...
while change!=0:
    ...
    for M1 in Matrizes:
        for M2 in MatrizesVelhas:
            Mprod.append(np.dot(M1,M2))
    for mp in Mprod:
        value=1
        for mv in MatrizesVelhas:
            if np.array_equal(mp,mv):
                value=0
        if value==1:
            v=1
            for mi in Matrizesinterm:
                if np.array_equal(mp,mv):
                    v=0
            if v==1:
                Matrizesinterm.append(mp)

    change = len(MatrizesVelhas)-len(Matrizesinterm)
    MatrizesVelhas = Matrizesinterm
    M.extend(Matrizesinterm)
    print len(M)
