...
def subgroupmaker_recursive(size,n, filenameopen):

    Dic_of_subgroups = {}
    List_of_subgroups = []
    listsubappend = List_of_subgroups.append

    Sub = subgroups_fromfile(filenameopen,size)
    for subgroups in Sub:
        for pos in range(len(subgroups)):
            A = list(subgroups)
            A[pos]=0
            if sum(tuple(A))==size-n:
                listsubappend([tuple(A),Sub[subgroups][1]])
    for element in List_of_subgroups:
        for perm in S:
            tp = tuple(perm(element[0]))
            if tp in Dic_of_subgroups: # in the keys!
                Dic_of_subgroups[tp].append(element[1])
                break
        else:
            Dic_of_subgroups[tuple(element[0])] = [element[1]]

    return Dic_of_subgroups

def subgroups_tofile(subgroups, filename, n):
    with open(filename, 'w') as f:
        print("# Removing {}. Subgroups: {}".format(n, len(subgroups)), file=f)
        for subgroup, origin in subgroups.items():
            i_sub = class2int(subgroup)
            print('{}\t{}'.format(i_sub, list(set(origin))), file=f)
    ...
    with open(filename) as f:
        ...
        for line in f:
            ...
            if line.startswith('#') or len(line) == 0:
                continue
            i_sub, origin = line.split('\t')
            numeration += 1
            subgroups[tuple(int2class(int(i_sub), size))] = [literal_eval(origin.strip()),numeration]
    return subgroups
    ...
    with open(filename) as f:
        for line in f:
            ...
            if line.startswith('#') or len(line) == 0:
                continue
            i_sub, origin = line.split('\t')
            yield tuple(int2class(int(i_sub), size)), literal_eval(origin.strip())
    return
...
def test():
    ...
    for size, n in (20,0), (20, 2), (20, 3):
        nexpected = comb(size-1, n-1, exact=True)

        print(fstring(size, n, nexpected))
...
## -> class 99900 / 5006386   76.24 s  2.00 %
## ===== 100000 classes, 69669 subgroups =====
