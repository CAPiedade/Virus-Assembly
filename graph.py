...
plot(f, layout=f.layout('rt_circular', mode = 'OUT', root=range(5)+sorted([5,9,22,21,41,40,49,48,33,32])+sorted([6,8,23,20,42,44,45,47,34,31])+sorted([7,53,24,29,43,12,46,35,30,56])+sorted([55,54,52,25,28,13,11,36,39,57])+sorted([50,51,26,27,14,10,37,38,58,59])+sorted([15,16,17,18,19]), rootlevel=[0]*5+[1]*10+[2]*10+[3]*10+[4]*10+[5]*10+[6]*5), bbox=(1000,1000))
...
def graphsymmetry(n):
    Lista=[]
    filename = "GroupsSymmetry"+str(n)+'.txt'
    for subgroup, n_subgroup in subgroups_fromfile_generator(filename, 60):
        ...
        if not f.is_connected():
            ...
            yield tuple([n_subgroup,tuple(f.vs['number']),len(deletelist2),tuple(sorted(deletelist)),tuple(sorted(deletelist2)),subgroup])
        else:
            yield tuple([n_subgroup,tuple(f.vs['number']),len(deletelist),tuple(sorted(deletelist)),tuple(sorted(deletelist)),subgroup])

#for n in range(54,55):
#    F = open('GraphSymmetry'+str(n)+'.txt','w')
